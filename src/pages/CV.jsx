...
export default function CV(){
  ...
  const { ref: toolandskillRef, inView: toolandskillInView } = useInView({
    threshold: 0.5,
    triggerOnce: true,
  })
  return (
    <>
      <div className="text-slate-300 grid grid-cols-1 lg:grid-cols-2 mt-6 gap-x-16 gap-y-8">
        <div ref={cvRef} className="w-full">
          <img src={cv} alt="cv" className={`transition duration-[1s] ${cvInView ? "opacity-100" : "opacity-0"} w-full`} />
        </div>
        <div ref={toolandskillRef} className="overflow-hidden">
          <div className={`transform transition duration-[1s] ${toolandskillInView ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"} space-y-8`}>
